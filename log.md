...
## [2026-08-13] maintenance | Publication and Recovery Guard Hardening

- Live publication audit: extended the inventory check to require successful home, Site Navigator and representative concept routes; exact deployed-versus-local SHA-256 matches for `publish.css` and `publish.js`; and an isolated headless-browser exercise of real `cash` type-ahead suggestions, lazy-loaded reader navigation labels, the Site Navigator shortcut and the welcome-page footer.
- Evidence integrity: replaced loose filename and hash searches with exact one-to-one validation across every `wiki/sources/` page, its `source_file` frontmatter link, its `source_hash`, the raw snapshot's calculated SHA-256 and the corresponding `raw/manifest.md` row. Registry cardinality, duplicate paths, duplicate source pages and path casing are now enforced.
- Recovery integrity: removed pruning and forced ref updates from the bare-mirror fetch. Current source refs must match exactly, while archive-only refs remain preserved and valid; an isolated three-generation fixture confirmed that a tag deleted from the source remained recoverable from the mirror.
- Credential safety: retained complete-history scanning but changed failures to display only redacted commit, file and line locations, never the detected value or matching line.
